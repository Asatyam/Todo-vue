...
watchEffect(() => {
  ...
  axios
    .get(
      `https://todo-api-58q4.onrender.com/api/projects/${props.current}`,
      config
    )
    .then((res) => {
...
const addNewTodo = () => {
  ...
  axios
    .post(
      `https://todo-api-58q4.onrender.com/api/projects/${props.current}/todos`,
      body,
      config
    )
    .then((res) => {
...
<template>
  <div class="p-3">
    <form @submit.prevent="addNewTodo">
      <div
        class="rounded grid grid-cols-[max-content_1fr_max-content_max-content] gap-4 bg-indigo-700 p-2 pl-4 pr-4 ml-auto mr-auto left-0 right-0 mb-3"
      >
        ...
        <input
          ...
          class="outline-none bg-indigo-500 pl-2 pr-2 rounded text-white"
        />
        <select
          v-model="priority"
          class="outline-none bg-indigo-500 pl-2 pr-2 rounded text-white"
    ...
    <Transition>
      <div
        v-if="showOptions"
        class="bg-white absolute top-21 left-50 text-black w-50"
      >
        ...
        <button
          class="block border-slate-500 border-b-2 pr-3 pl-2 text-sm text-start py-1 w-full font-light hover:bg-blue-700 hover:text-white transition-colors"
          @click="deleteProject"
        >
          Delete Project
        </button>
